  ...
  outputs = { self, nixpkgs, flake-utils }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        pythonEnv = pkgs.python3.withPackages (ps: [ ps.gnureadline ]);

        setup-model-dir = pkgs.stdenv.mkDerivation {
          pname = "setup-model-dir";
          version = "0.1.0";
          src = ./.;
          ...
          installPhase = ''
            mkdir -p $out/bin
            cp setup-model-dir.py $out/bin/setup-model-dir
            chmod +x $out/bin/setup-model-dir
            wrapProgram $out/bin/setup-model-dir \
      ...
      in
      {
        packages = {
          ...
          setup-model-dir = setup-model-dir;
        };
